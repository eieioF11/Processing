...
float[] AveX = new float[64];
float[] AveY = new float[64];
void NDT()
{
	for(int x=0;x<64;x++)
	{
		for(int y=0;y<64;y++)
		{
			...
			{
				if(fielddata[i*grid][y*grid]!=0)
					AveX[x]+=1;
				AveX[x]/=grid;
				String data="x,"+nf(x)+","+nf(AveX[x],3,3);
				text(data,i*grid,y*grid+10);
			}
			for(int i=y;i<y+grid;i++)
			{
				if(fielddata[x*grid][i*grid]!=0)
					AveY[y]+=1;
				AveY[y]/=grid;
				String data="y,"+nf(y)+","+nf(AveY[y],3,3);
				text(data,x*grid,i*grid+20);
			}
		}
	}
}
